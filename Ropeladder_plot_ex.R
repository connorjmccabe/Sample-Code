...
require(dplyr)
rummj_smw.paramscsv<-read.csv("sampleparams.csv")

head(rummj_smw.paramscsv)

require(dplyr)
rummj_smw.params.clean<-rummj_smw.params %>%
  mutate(param=as.character(param)) %>%
  # This recodes the ordinal axis to something more sensible compared to lavaan output. Just 2 examples for now:
